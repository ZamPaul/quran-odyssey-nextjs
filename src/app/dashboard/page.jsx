    ...
    <div style={{ background: 'white', borderRadius: 12, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
      <button onClick={onToggle} style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', padding: '16px 20px', background: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 4 }}>
            ...
            {assignmentAttachments(assignment).length > 0 && <span style={{ fontSize: 11, fontWeight: 600, color: '#94a3b8', background: '#f0f4f8', borderRadius: 4, padding: '2px 7px' }}>📎 {assignmentAttachments(assignment).length} file{assignmentAttachments(assignment).length === 1 ? '' : 's'}</span>}
          </div>
      ...
      {expanded && (
        <div style={{ borderTop: '1px solid #e2e8f0', padding: '20px', background: '#fafbfc' }}>
          ...
          {assignmentAttachments(assignment).length > 0 && (
            <FileList files={assignmentAttachments(assignment)} label="Teacher's attachments" />
          )}
          ...
          {sub && isGraded && (
            <div style={{ marginTop: 14, padding: '14px 16px', borderRadius: 8, background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.2)' }}>
              ...
              {submissionFiles(sub).length > 0 && <div style={{ marginTop: 10 }}><FileList files={submissionFiles(sub)} label="Your submitted files" /></div>}
            </div>
          )}
          ...
          {sub && !isGraded && !editing && (
            <div style={{ marginTop: 14 }}>
              ...
              {submissionFiles(sub).length > 0 && <FileList files={submissionFiles(sub)} label="Submitted files" />}
          ...
          {!sub && assignment.status !== 'GRADED' && (
            <div style={{ marginTop: assignmentAttachments(assignment).length ? 16 : 0 }}>
...
function SubmitForm({ assignmentId, studentId, onSuccess, onCancel, mode = 'create', existing = null }) {
  ...
  const [files,   setFiles]   = useState(submissionFiles(existing));
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div>
        <label style={{ fontSize: 12, fontWeight: 700, color: '#64748b', display: 'block', marginBottom: 6 }}>
          File Upload <span style={{ fontWeight: 400, color: '#94a3b8' }}>(optional — images, PDFs, audio recordings)</span>
        </label>
        <MultiFileUpload
          role="student" userId={userId} label="Upload files with this submission" compact
          max={10}
          files={files}
          onChange={(next) => { setFiles(next); setError(''); }}
        />
      </div>
      ...
      <main style={{ marginLeft: 248, flex: 1, display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
        ...
        <div style={{ background: 'white', borderBottom: '1px solid #e2e8f0', padding: '0 28px', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 40 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {activeStudent && (
              <Link href={`/learn/${activeStudent.id}`} title="Open the child's gamified journey" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'linear-gradient(90deg,#FF7EB6,#8B5CF6)', color: '#fff', padding: '7px 16px', borderRadius: 999, fontSize: 13, fontWeight: 800, border: 'none', cursor: 'pointer', textDecoration: 'none', boxShadow: '0 3px 0 #b45aa8' }}>
                ✨ Journey
              </Link>
            )}
